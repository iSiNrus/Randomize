...
#ifdef SUPERVERBOSE
//        qDebug() << "HttpRequest: receive body" << newData.size() << currentSize << expectedBodySize;
#endif
        //Receiving binary file
        if (getHeader("Content-Type") == "application/octet-stream") {
          if (tempFile == NULL)
          {
          ...
          if (bodyBuffer.size() + newData.size() > FILE_UPLOAD_BUFFER) {
#ifdef SUPERVERBOSE
            qDebug() << "Writing to temp:" << bodyBuffer.size() << currentSize << expectedBodySize;
#endif
            tempFile->write(bodyBuffer);
            bodyBuffer.clear();
          }
          bodyBuffer.append(newData);

          if (currentSize >= expectedBodySize) {
            tempFile->write(bodyBuffer);
            tempFile->flush();
            tempFile->close();
            uploadedFiles.insert(QString(REQUEST_FILE_ID).toLatin1(), tempFile);
            tempFile = NULL;
            status = complete;
          }
        }
